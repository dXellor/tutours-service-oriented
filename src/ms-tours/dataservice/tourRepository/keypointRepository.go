...
func (repo *KeypointRepository) Get(id int) (model.Keypoint, error) {
...
func (repo *KeypointRepository) GetByTour(tourId int) ([]model.Keypoint, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (repo *KeypointRepository) Update(keypoint *model.Keypoint) (model.Keypoint, error) {
	...
	updatedKeypoint, err := repo.Get(keypoint.Id)
	if err != nil {
		return model.Keypoint{}, err
	}
