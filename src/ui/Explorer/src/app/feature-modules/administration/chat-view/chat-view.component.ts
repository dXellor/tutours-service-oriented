...
export class ChatViewComponent implements OnInit {
  ...
  public followerCtrl: FormControl<Profile | null> =
    new FormControl<Profile | null>(null);
  public followerFilterCtrl: FormControl<string | null> =
    new FormControl<string>('');
  public filteredFollowers: ReplaySubject<Profile[]> = new ReplaySubject<
    Profile[]
  >(1);
  ...
  private createAdvertisementMessage(): void {
    let adMessage: ChatMessage = {
      ...
      sender: {
        ...
        userId: -1,
        profileImage:
          'https://avatars.githubusercontent.com/u/65507197?s=200&v=4',
      },
    };
  ...
  private filterFollowers() {
    ...
    this.filteredFollowers.next(
      this.followers.filter(
        (follower) =>
          follower.name.toLowerCase().indexOf(search || '') > -1 ||
          follower.surname.toLowerCase().indexOf(search || '') > -1
      )
    );
  }
